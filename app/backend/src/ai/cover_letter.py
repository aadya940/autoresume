...
if not os.getenv("GOOGLE_API_KEY"):
    ...
    raise ValueError(
        "GOOGLE_API_KEY is required for cover letter generation. Please set it in your .env file."
    )
...
class CoverLetterGenerator:
    ...
    async def _generate_async(
        self, job_description: str, company: str, title: str, resume_path: Path
    ) -> Dict[str, any]:
...
Keep total output to 400-500 words."""
        ...
        if not template_path.exists():
            ...
            logger.warning(
                f"Template not found at {template_path}, creating default template"
            )
